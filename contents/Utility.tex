...
$$
{Overall\ utility = utility\ of\ recipe + expiry\ score}
$$

\subsection{Utility of recipe}
The utility of each recipe is modeled as a function of multiple attributes, including the estimated cooking time, ease of preparation, nutritional profile, dietary classification (e.g., vegetarian), and course type (e.g., dessert). We apply the \textit{k}\nobreakdash-means clustering algorithm to group recipes with similar attribute profiles and assign a utility value to each cluster. Model training was conducted on an online Kaggle dataset (\cite{RecipeDataset}).
...
\input{appendix/algorithms/algo0}
In order to have a large diversity for the recipe utility, we set the number of clusters to $k = 2000$. The k-means algorithm is trained using $scikit-learn$ implementation. The trained model is saved into a ".joblib" file for future predictions.\par
