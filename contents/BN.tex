...
The \textbf{feasibility} of preparing recipe $\recipe_i$ depends on state of foods available in the \textbf{fridge} that is in the requirement set $\requirements_{\recipe_i}$. We assume that the state of each food is dependent on three \textbf{\underline{independent}} characteristics of food $\food_i$: \textbf{food type} $\fty_{\food_i}$, \textbf{storage type} $\sty_{\food_i}$, and \textbf{days in the fridge} $\days_{\food_i}$. 
...
&=P(feasibility_{\recipe_i}  | expired_{\food_1}, \dots expired_{\food_{n}})\prod_{k=1}^nP(expired_{\food_i} | \fty_{\food_i}=f, \sty_{\food_i}=s, \days_{\food_i}=d) \\
